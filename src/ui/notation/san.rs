...
/// san_str can contain one or several moves: 'Nd5 a4' for example
/// Does not suit for epd reading since Nd5 could mean Ncd5 or Ngd5 for example.
pub fn san_to_long_notation(
    san_str: &str,
...
/// manage ambiguous notaiton
pub fn san_to_long_notation_epd(
    san_str: &str,
    moves: &Vec<bitboard::BitBoardMove>,
    lang: &Lang,
    game: &game_state::GameState,
    ...
    let raw_move: Vec<_> = moves
        ...
        .filter(|b_move| {
            ...
            // Nxe5 becomes Ne5
            san_moves.contains(&san_to_str(b_move, moves, lang, game, zobrist_table, true).info().replace("x", "").replace("+", "").replace("=", ""))
        })
        .map(|b_move| long_notation::LongAlgebricNotationMove::build_from_b_move(*b_move).cast())
        .collect();
    raw_move
...
fn is_move_check(
    ...
    let mv = vec![long_notation::LongAlgebricNotationMove::build_from_b_move(
        *b_move,
    )];
    let mut game_clone = game.clone();
    game_clone
        .play_moves(&mv, zobrist_table, None, true)
        .unwrap();
    game_clone.check_status().is_check()
}
/// When is_ambiguous = false, we make a distinction between Nce5 and Nge5
pub fn san_to_str(
    ...
    let san = san_to_str_no_check(move_to_translate, moves, lang, is_ambiguous);
...
#[cfg(test)]
mod tests {
    use crate::ui::notation::san::{san_to_long_notation_epd, san_to_str_no_check, Lang};
    use crate::{
    ...
    fn test_san_pawn_capture() {
        ...
        let result = san_to_str_no_check(&move_to_translate, &moves, &Lang::LangEn, false);
    ...
    fn test_san_pawn_2x() {
        ...
        let move_to_translate =
            bitboard::BitBoardMove::new(color, type_piece, start, end, capture, promotion);
        let moves = vec![move_to_translate];
        let result = san_to_str_no_check(&move_to_translate, &moves, &Lang::LangFr, false);
    ...
    fn test_san_to_long_notation() {
        ...
        // black long castle
        assert_eq!(
            san_to_long_notation("O-O-O", &moves, &Lang::LangFr, &game, &zobrist_table),
            Some("e8c8".to_string())
        );
        // Knight in f6
        assert_eq!(
            san_to_long_notation("Cf6", &moves, &Lang::LangFr, &game, &zobrist_table),
            Some("g8f6".to_string())
        );
        // Rook in d8
        assert_eq!(
            san_to_long_notation("Td8", &moves, &Lang::LangFr, &game, &zobrist_table),
            Some("a8d8".to_string())
        );
        // Pawn in e6
        assert_eq!(
            san_to_long_notation("e6", &moves, &Lang::LangFr, &game, &zobrist_table),
            Some("e7e6".to_string())
        );
        ...
        assert_eq!(
            san_to_long_notation("Fxc7", &moves, &Lang::LangFr, &game, &zobrist_table),
            Some("f4c7".to_string())
        );
        // White Bishop to b5
        assert_eq!(
            san_to_long_notation("Fb5", &moves, &Lang::LangFr, &game, &zobrist_table),
            Some("f1b5".to_string())
        );
        // White pawn to h4
        assert_eq!(
            san_to_long_notation("h4", &moves, &Lang::LangFr, &game, &zobrist_table),
            Some("h2h4".to_string())
        );
        ...
        assert_eq!(
            san_to_long_notation("Cge7", &moves, &Lang::LangFr, &game, &zobrist_table),
            Some("g8e7".to_string())
        );
        // ambiguous notation
        let result = san_to_long_notation_epd("Ce7", &moves, &Lang::LangFr, &game, &zobrist_table);
        assert!(result.contains(&"g8e7".to_string()));
        assert!(result.contains(&"c6e7".to_string()));
        let mv = vec![long_notation::LongAlgebricNotationMove::build_from_str("g8e7").unwrap()];
        game.play_moves(&mv, &zobrist_table, None, false).unwrap();
        let moves = game.gen_moves();
        // white short castle
        assert_eq!(
            san_to_long_notation("O-O", &moves, &Lang::LangFr, &game, &zobrist_table),
            Some("e1g1".to_string())
        );
        // Bishop take queen
        assert_eq!(
            san_to_long_notation("Fxa2", &moves, &Lang::LangFr, &game, &zobrist_table),
            Some("c4a2".to_string())
        );
        ...
        // pawn
        assert_eq!(
            san_to_long_notation("g6", &moves, &Lang::LangFr, &game, &zobrist_table),
            Some("g7g6".to_string())
        );
        ...
        let moves = game.gen_moves();
        assert_eq!(
            san_to_long_notation("Fxc6+", &moves, &Lang::LangFr, &game, &zobrist_table),
            Some("b5c6".to_string())
        );
    }
}
