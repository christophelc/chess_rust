...
impl Board for ChessBoard {
    fn set_initial_position(&mut self) {
        ...
        self.add(coord::Coord::from('A', 1).unwrap(), square::TypePiece::Rook, square::Color::White);
        self.add(coord::Coord::from('B', 1).unwrap(), square::TypePiece::Knight, square::Color::White);    
        self.add(coord::Coord::from('C', 1).unwrap(), square::TypePiece::Bishop, square::Color::White);        
        self.add(coord::Coord::from('D', 1).unwrap(), square::TypePiece::Queen, square::Color::White);        
        self.add(coord::Coord::from('E', 1).unwrap(), square::TypePiece::King, square::Color::White);            
        self.add(coord::Coord::from('F', 1).unwrap(), square::TypePiece::Bishop, square::Color::White);                
        self.add(coord::Coord::from('G', 1).unwrap(), square::TypePiece::Knight, square::Color::White);
        self.add(coord::Coord::from('H', 1).unwrap(), square::TypePiece::Rook, square::Color::White);                    
        for col in 'A'..='H' {
            self.add(coord::Coord::from(col, 2).unwrap(), square::TypePiece::Pawn, square::Color::White);                        
        }

        self.add(coord::Coord::from('A', 8).unwrap(), square::TypePiece::Rook, square::Color::Black);
        self.add(coord::Coord::from('B', 8).unwrap(), square::TypePiece::Knight, square::Color::Black);    
        self.add(coord::Coord::from('C', 8).unwrap(), square::TypePiece::Bishop, square::Color::Black);        
        self.add(coord::Coord::from('D', 8).unwrap(), square::TypePiece::Queen, square::Color::Black);        
        self.add(coord::Coord::from('E', 8).unwrap(), square::TypePiece::King, square::Color::Black);            
        self.add(coord::Coord::from('F', 8).unwrap(), square::TypePiece::Bishop, square::Color::Black);                
        self.add(coord::Coord::from('G', 8).unwrap(), square::TypePiece::Knight, square::Color::Black);
        self.add(coord::Coord::from('H', 8).unwrap(), square::TypePiece::Rook, square::Color::Black);                    
        for col in 'A'..='H' {
            self.add(coord::Coord::from(col, 7).unwrap(), square::TypePiece::Pawn, square::Color::Black);
        }
    }
        ...
        for (i, row) in self.squares.iter().enumerate().rev() {
            ...
            for (j, square) in row.iter().enumerate() {
                ...
                let piece_color = match square {
                    square::Square::NonEmpty(piece) if piece.color() == square::Color::White => "\x1B[97m",  // White foreground
                    square::Square::NonEmpty(_) => "\x1B[96m",  // Black foreground
                    square::Square::Empty => "",
                };                
                let display_square = format!("{}", square);
                write!(f, "{}{}{} \x1B[0m", background_color, piece_color, display_square)?;
            }
