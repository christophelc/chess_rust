...
impl EngineAlphaBeta {
    ...
    fn alphabeta(
        ...
        let results: Vec<_> = chunks
            ...
            .map(|chunk| {
                ...
                let (best_move, score) = self.alphabeta_rec(
                    "",
                    &mut game_clone,
                    0,
                    &chunk,
    ...
    fn process_move(
        ...
        let score = if game.end_game() == game_state::EndGame::None {
            ...
            if current_depth < self.max_depth || m.capture().is_some() && current_depth < self.max_depth + 2 {
                moves = if current_depth < self.max_depth {
                    game.gen_moves()
                } else {
                    // just consider capture for last depth in case of capture
                    game.gen_moves().into_iter().filter(|b_move| b_move.capture().is_some()).collect()
                };
            }
            if current_depth < self.max_depth || m.capture().is_some() && !moves.is_empty() {
                let (_, score) = self.alphabeta_rec(
                    &updated_variant,
                    game,
                    current_depth + 1,
                    &moves,
